                ...
                object : SocialLoginCallback {
                    override fun onSuccess(idToken: String) {
                        lifecycleScope.launch {
                            try {
                                val authService =
                                    NetworkModule.provideSocialLoginApi(this@LoginActivity)
                                val response =
                                    authService.loginWithKakao(
                                        LoginRequest(idToken)
                                    )
                                Log.d("LoginActivity", "카카오 로그인 성공: $response")
                                tokenStore.saveTokens(response.accessToken, response.refreshToken)
                                startActivity(
                                    Intent(this@LoginActivity, TokenDebugActivity::class.java)
                                )
                            } catch (e: Exception) {
            ...
            registerForActivityResult(
                ...
                if (result.resultCode == RESULT_OK) {
                    GoogleLoginHelper.handleResult(
                        context = this, data = result.data,
                        callback =
                            object : SocialLoginCallback {
                                override fun onSuccess(idToken: String) {
                                    lifecycleScope.launch {
                                        try {
                                            val authService = NetworkModule.provideSocialLoginApi(this@LoginActivity)
                                            val response = authService.loginWithGoogle(
                                                        LoginRequest(idToken)
                                                    )
                                            Log.d("LoginActivity", "구글 로그인 성공: $response")
                                ...
                                override fun onFailure(errorMessage: String?) {
                                    Toast.makeText(this@LoginActivity, "Google 로그인 실패: $errorMessage", Toast.LENGTH_SHORT).show()
                                }
                            }
                    )
                } else {
